...
module.exports = {
  ...
  async execute(message, args, cmd, client, discord, profileData) {
    if (!args.length) return message.channel.send("You need to mention a member before you can increase their level.");
    const amount = args[1];
    ...
    try {
      ...
      return message.channel.send(`${message.author.username}, the targeted member has been given \`${amount}\` amount of levels.`);
    } catch (err) {
      console.log(err);
    }
  },
