...
module.exports = {
  ...
  async execute(message, args, cmd, client, Discord, profileData) {
    if (!args.length)
      return message.channel.send({ content: "Om het eerlijk te houden, moet je ook jouw keuze invoeren!"});
    ...
    if (!list.includes(args[0].toLowerCase())) {
      return message.channel.send({ content: `\`${args[0]}\` is geen geldige keuze!`});
    }
